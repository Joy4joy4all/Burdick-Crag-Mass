# -*- coding: utf-8 -*-
"""
BCM v20.5 -- Stellar Gate (Chiral L1 Crossing)
=================================================
Stephen Justin Burdick Sr., 2026 -- Emerald Entities LLC
GIBUSH Systems

v20.1-v20.4 modeled the craft crossing the stellar
boundary as PA entering first, PB second — two points,
sequential transit. That's wrong.

Stephen Burdick Sr.'s correction:
  "The craft crosses at L1. The entire 6D field is a
   ribbon. The ribbon folds at L1. The craft rides the
   chiral edge at 12,000c which cambers the loop. The
   shadow through that loop creates two opacity patterns:
   C-shadow (spatial) and T-shadow (temporal). Two phase
   clocks. Phase clocks are operators of pump intent to
   create inertia."

The circumpunct (point within circle) is L1 within the
ribbon loop. At 12,000c the craft outruns the star's
substrate refresh rate. The shadow blows out the candle
before it can flicker.

THIS TEST: Replace separate PA/PB crossing with L1
chiral fold transit. The craft enters the stellar field
as ONE object through L1. The chiral phase offset
replaces the spatial gradient. The ribbon curl at L1
determines emergence side.

New frozen constant:
  CHIRAL_COUPLING = 0.12 (ribbon fold intensity at L1)

Usage:
    python BCM_v20_stellar_gate_v5.py
    python BCM_v20_stellar_gate_v5.py --steps 20000 --grid 256
"""
...
def main():
    ...
    for step in range(steps):
        ...
        if step%diag_interval==0:
            ...
            flight_log.append({
                ...
                "grad_shadow": round(sig_at_l1, 4),
                "shadow_active": chiral_active,
                "chiral_offset": round(chiral_offset, 4)
                    if chiral_active else 0.0,
    ...
    out_path=os.path.join(out_dir,
        f"BCM_v20_stellar_gate_v5_"
        f"{time.strftime('%Y%m%d_%H%M%S')}.json")

    out_data={
        "title":"BCM v20.5 Stellar Gate (Chiral L1 Crossing)",
