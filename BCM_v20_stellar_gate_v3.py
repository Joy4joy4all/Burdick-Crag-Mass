# -*- coding: utf-8 -*-
"""
BCM v20.3 -- Stellar Gate (Active Phase Authority)
=====================================================
Stephen Justin Burdick Sr., 2026 -- Emerald Entities LLC
GIBUSH Systems

v20.2 gate detected the star but couldn't hold the
heartbeat. PhiRMS still climbed because the gate only
clipped the pump — it didn't actively REJECT the
external rhythm. The star recruited the craft's clock.

Four AIs converged:
  ChatGPT: "You're building an inverse PLL — a system
    that must refuse lock."
  Gemini: "Active Phase Decorrelation. Convert the star's
    shout into chi pressure. Own the frequency of the
    hallway."
  Grok: "Phase authority >= external coupling rate.
    That's the last gate."

THIS TEST: Add Active Phase Decorrelation (APD) layer.
The craft doesn't just detect the star — it COUNTER-
FORCES the phase intrusion at every step. Continuous
rejection, not event-based correction.

Three APD mechanisms:
  1. PD controller (KP + KD) applies counter-force
  2. Pump authority collapses proportional to phase error
  3. Chi flywheel absorbs phase error energy (not vent)

Frozen APD constants (no per-object tuning):
  KP_PHASE = 0.85 (proportional rejection)
  KD_PHASE = 0.40 (derivative damping)
  CHI_STABILIZER_GAIN = 12.5 (flywheel conversion)

Usage:
    python BCM_v20_stellar_gate_v3.py
    python BCM_v20_stellar_gate_v3.py --steps 20000 --grid 256
"""
...
def main():
    ...
    out_path=os.path.join(out_dir,
        f"BCM_v20_stellar_gate_v3_"
        f"{time.strftime('%Y%m%d_%H%M%S')}.json")

    out_data={
        "title":"BCM v20.3 Stellar Gate (Active Phase Authority)",
