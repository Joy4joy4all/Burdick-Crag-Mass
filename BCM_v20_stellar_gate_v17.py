...
def get_stellar_profile(step, total_steps):
    """
    BALLISTIC transit through Alpha Centauri A.
    At 12,000c the entire stellar crossing is a flash.
    The craft doesn't negotiate. It punches through.

    The star appears and disappears in ~50 steps.
    The rest is void approach and void recovery.
    Dwell time is the enemy. Velocity is armor.

    Returns: (lambda, stellar_pump_amplitude, phase_name)
    """
    # At 12,000c, the stellar encounter is compressed
    # to ~50 steps centered at the midpoint
    TRANSIT_DURATION = 50  # total steps inside star
    center = total_steps // 2
    entry_start = center - TRANSIT_DURATION // 2
    entry_end = center + TRANSIT_DURATION // 2

    if step < entry_start:
        # Void approach — no star
        return 0.05, 0.0, "APPROACH"

    elif step < entry_start + 10:
        # Flash entry (10 steps)
        t = (step - entry_start) / 10.0
        lam = 0.05 - t * 0.045
        pump = t * 2.0
        return lam, pump, "ENTRY"

    elif step < entry_start + 20:
        # Tachocline crossing (10 steps)
        t = (step - entry_start - 10) / 10.0
        lam = 0.005 - t * 0.004
        pump = 2.0 + t * 3.0
        return lam, pump, "TACHOCLINE"

    elif step < entry_start + 30:
        # Core flash (10 steps — max pump)
        lam = 0.001
        pump = 5.0
        return lam, pump, "CORE"

    elif step < entry_start + 40:
        # Core exit (10 steps)
        t = (step - entry_start - 30) / 10.0
        lam = 0.001 + t * 0.004
        pump = 5.0 - t * 3.0
        ...
        # Flash exit (10 steps)
        t = (step - entry_start - 40) / 10.0
        lam = 0.005 + t * 0.045
        pump = 2.0 - t * 2.0
        return lam, pump, "EXIT"

    else:
        # Void recovery — star is gone
...
def main():
    ...
    out_path=os.path.join(out_dir,
        f"BCM_v20_stellar_gate_v17_"
        f"{time.strftime('%Y%m%d_%H%M%S')}.json")

    out_data={
        "title":"BCM v20.17 Stellar Gate (Silent Recovery)",
