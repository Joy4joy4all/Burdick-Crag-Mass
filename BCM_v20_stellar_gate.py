# -*- coding: utf-8 -*-
"""
BCM v20.1 -- Stellar Gate Transit (Tachocline Gate)
=====================================================
Stephen Justin Burdick Sr., 2026 -- Emerald Entities LLC
GIBUSH Systems

v20 stellar transit showed craft stress inside Alpha
Centauri's substrate field. The star's m=1 tachocline
pump beats against the craft's H10 pump mode.

THIS TEST: Add the tachocline gate detector (7D fold
matrix). The craft reads the incoming stellar pump
signature BEFORE physical overlap and governs the fold:

  1. Telescopic bridge stretch (detune separation)
  2. Core-drop safety bleed into chi freeboard
  3. Phase realignment slew (gentle lambda detune)

The gate is the ship looking at itself from the
approach angle: "will this star's memory overwrite
mine?" The 7D fold matrix answers that question
before the 6D blast patterns overlap.

Three frozen gate constants (no per-object tuning):
  GATE_THRESHOLD = 0.15 (phase proximity trigger)
  BRIDGE_STRETCH = 1.12 (temporary separation increase)
  CORE_DROP_FRAC = 0.35 (one-time chi bleed fraction)

Usage:
    python BCM_v20_stellar_gate.py
    python BCM_v20_stellar_gate.py --steps 20000 --grid 256
"""
...
def main():
    ...
    for step in range(steps):
        ...
        # ---- TACHOCLINE GATE DETECTOR (7D FOLD) ----
        # The ship reads the incoming stellar pump mode
        # before physical overlap. If the star's m-value
        # phase-locks with the craft's pump mode, the
        # gate triggers and governs the fold.
        #
        # Alpha Centauri A: m=1 (from v4 tachocline)
        # Craft pump: H10 at 0.200 cycles/step
        # Beat: |H10 - m_star * f_probe| proximity
        star_m = 1  # Alpha Centauri A tachocline mode
        craft_h10 = 0.200  # pump mode
        star_freq = star_m * 0.020  # m * probe fundamental
        phase_proximity = abs(craft_h10 - star_freq * 10)

        # Gate triggers when stellar pump is active AND
        # phase proximity is within threshold
        if (stellar_pump > 0.5 and
                phase_proximity < GATE_THRESHOLD and
                not gate_active):
            gate_active = True
            gate_step = step
            gate_events.append({
        ...
        # Gate closes when stellar pump drops below 0.5
        if gate_active and stellar_pump < 0.3:
            gate_active = False
            core_dropped = False  # reset for next encounter
            separation = base_separation  # restore
            gate_events.append({
                "step": step, "phase": phase,
                "action": "GATE_CLOSE",
            })

        # ---- GOVERNED FOLD RESPONSE ----
        if gate_active:
            # 1. Telescopic bridge stretch (detune)
            separation = base_separation * BRIDGE_STRETCH

            # 2. Core-drop: one-time safety bleed
            if not core_dropped:
                # Bleed orbital sigma from all probes
                # directly into chi freeboard
                for p in probes:
                    if p.payload > 0:
                        drop = CORE_DROP_FRAC * p.payload
                        p.payload -= drop
                        p.total_bled += drop
                        # Route to chi at craft center
            ...
            # 3. Lambda gradient damping (gentle slew)
    ...
    out_path=os.path.join(out_dir,
        f"BCM_v20_stellar_gate_"
        f"{time.strftime('%Y%m%d_%H%M%S')}.json")

    out_data={
        "title":"BCM v20.1 Stellar Gate Transit",
