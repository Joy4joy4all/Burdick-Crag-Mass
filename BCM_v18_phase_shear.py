# -*- coding: utf-8 -*-
"""
BCM v18 -- Fractal Phase Shear (Mode Killer)
===============================================
Stephen Justin Burdick Sr., 2026 -- Emerald Entities LLC
GIBUSH Systems

ChatGPT directive: "You are redistributing amplitude,
not collapsing coherence. Move from drain to phase
scrambling."

The Brucetron mode survives because all regions oscillate
in lockstep. Fractal boundary provides non-uniform
geometry. Position-dependent phase disruption derived
from fractal curvature breaks global coherence.

Psi = (D_f - 1) * |grad(probe_density)|
phi *= (1 - k_shear * Psi)

Where Psi varies spatially: high at fractal boundary
edges (rapid curvature change), low in smooth regions.
This breaks lockstep because different positions get
different attenuation based on local fractal geometry.

FOUR RUNS:
  A: No dissipation (baseline)
  B: Chi tank (v17)
  C: Phase projection only (v18 — k1 drain)
  D: Phase shear (v18 final — k1 drain + k_shear)

Kill condition: phi RMS must drop significantly in D
vs C. If it does, the eigenmode is broken.

Usage:
    python BCM_v18_phase_shear.py
...
def run_config(nx, ny, steps, dt, D, alpha, separation,
               ...
               mode, k1=0.002, k_shear=0.1, rng_seed=42):
    ...
    probes = [ShearProbe(i + 1, i * 5) for i in range(12)]
    ...
    for step in range(steps):
        ...
        if mode == 'chi_tank':
            ...
            # TERM 1: magnitude drain (existing v18)
            grad_phi_x = np.roll(phi, -1, 0) - np.roll(phi, 1, 0)
            grad_phi_y = np.roll(phi, -1, 1) - np.roll(phi, 1, 1)
            grad_phi_sq = grad_phi_x**2 + grad_phi_y**2
            weight = np.power(density + 1e-10, 0.5)
            ...
            # TERM 2: fractal phase shear (NEW)
            # Psi = (D_f - 1) * |grad(probe_density)|
            # Position-dependent: high at fractal edges,
            # low in smooth regions. Breaks lockstep.
            grad_dens_x = np.roll(density, -1, 0) - np.roll(density, 1, 0)
            grad_dens_y = np.roll(density, -1, 1) - np.roll(density, 1, 1)
            grad_dens_mag = np.sqrt(grad_dens_x**2 + grad_dens_y**2)
            D_f = 1.5
            psi_shear = (D_f - 1.0) * grad_dens_mag
            # Shear factor: varies spatially from 1.0 (no
            # disruption) to (1 - k_shear*psi) at fractal edges
            shear_factor = 1.0 - k_shear * psi_shear
            shear_factor = np.clip(shear_factor, 0.0, 1.0)
            # Apply: magnitude drain + phase shear
            phi -= phi_dissip
            phi *= shear_factor
            ...
            dissipated_this_step = (float(np.sum(phi_dissip)) +
                float(np.sum(phi * (1.0 - shear_factor))) +
                float(np.sum(bruce_proj)))
...
def main():
    parser = argparse.ArgumentParser(
        description="BCM v18 Fractal Phase Shear")
    ...
    print(f"\n{'='*65}")
    print(f"  BCM v18 -- FRACTAL PHASE SHEAR")
    print(f"  Break lockstep, not just drain amplitude")
    ...
    configs = [
        ...
        ("D: Phase shear (v18 final)", "phase_shear"),
    ]
    ...
    print(f"\n{'='*65}")
    print(f"  PHASE SHEAR COMPARISON")
    print(f"{'='*65}")
    ...
    print(f"    C (projection):      {red_c:.1f}%")
    print(f"    D (phase shear):     {red_d:.1f}%")

    print(f"\n  Phi RMS comparison (MODE TEST):")
    print(f"    A (baseline):        {phi_a:.8f}")
    print(f"    C (projection):      {phi_c:.8f}")
    print(f"    D (phase shear):     {phi_d:.8f}")

    if phi_d < phi_c * 0.9:
        phi_drop = (1 - phi_d / phi_c) * 100
        print(f"    MODE BROKEN: {phi_drop:.1f}% phi RMS drop")
    elif phi_d < phi_c * 0.98:
        phi_drop = (1 - phi_d / phi_c) * 100
        print(f"    Partial mode disruption: {phi_drop:.1f}%")
    else:
        print(f"    Mode persists — phase shear insufficient")

    print(f"\n{'='*65}")
    print(f"  PHASE SHEAR VERDICT")
    print(f"{'='*65}")

    if rate_d < rate_c and phi_d < phi_c * 0.9:
        print(f"\n  MODE DESTRUCTION CONFIRMED")
        print(f"  Fractal phase shear breaks eigenmode coherence.")
        print(f"  The Frastrate is a phase decorrelator.")
    elif rate_d < rate_c:
        print(f"\n  IMPROVED RATE, mode partially disrupted")
    elif phi_d < phi_c * 0.9:
        print(f"\n  MODE DISRUPTED but growth rate unchanged")
        print(f"  Coherence broken but energy still injected")
    else:
        print(f"\n  OPEN QUESTION: mode resistant to phase shear")
        print(f"  Brucetron eigenmode is phase-coherent and")
        print(f"  resistant to scalar dissipation AND shear.")
        print(f"  Publish as open problem.")
    ...
    out_path = os.path.join(out_dir,
        f"BCM_v18_phase_shear_"
        f"{time.strftime('%Y%m%d_%H%M%S')}.json")

    out_data = {
        "title": "BCM v18 Fractal Phase Shear",
        "author": ("Stephen Justin Burdick Sr. -- "
                    "Emerald Entities LLC"),
        "purpose": ("Phase decorrelation via fractal curvature "
                     "gradient — break eigenmode lockstep"),
        "grid": nx, "steps": args.steps,
        "configs": all_results,
        "rate_reduction": {
            "chi_tank": round(red_b, 2),
            "projection_only": round(red_c, 2),
            "phase_shear": round(red_d, 2),
        },
    }
