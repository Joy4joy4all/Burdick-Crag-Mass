# -*- coding: utf-8 -*-
"""
BCM v20.2 -- Stellar Gate Transit (7D Spectral Fold)
======================================================
Stephen Justin Burdick Sr., 2026 -- Emerald Entities LLC
GIBUSH Systems

v20.1 gate was amplitude-blind. The star's m=1 tachocline
imposed its rhythm on the craft's f/2 heartbeat before the
gate detected it. PhiRMS detonated from 0.001 to 0.08.
Disorientation abort at step 3318.

Diagnosis (ChatGPT + Gemini + Grok converged):
  "3D stellar bodies are not barriers — they are dominant
   oscillators. You don't need shielding. You need phase
   governance."

THIS TEST: Replace amplitude gate with 7D spectral fold
detector. The craft watches the RATE of phase change
(dphi/dt) and the spectral proximity of the external
pump. Gate fires on the ramp, not the spike.

Three new mechanisms:
  1. dphi rate trigger (fire on ramp, not peak)
  2. Pre-coupling pump governor (clip pump before 0.2)
  3. Fast chi bleed (shock absorber, not passive decay)

Frozen gate constants (no per-object tuning):
  DPHI_GATE = 0.012 (rate trigger)
  PHASE_LOCK_THRESHOLD = 0.18 (spectral proximity)
  PUMP_CLIP = 0.55 (immediate decouple ratio)
  CHI_SHOCK = 0.82 (fast bleed multiplier)

Usage:
    python BCM_v20_stellar_gate_v2.py
    python BCM_v20_stellar_gate_v2.py --steps 20000 --grid 256
"""
...
# v20.2: spectral fold detector, not amplitude gate
...
def main():
    ...
    out_path=os.path.join(out_dir,
        f"BCM_v20_stellar_gate_v2_"
        f"{time.strftime('%Y%m%d_%H%M%S')}.json")

    out_data={
        "title":"BCM v20.2 Stellar Gate (7D Spectral Fold)",
