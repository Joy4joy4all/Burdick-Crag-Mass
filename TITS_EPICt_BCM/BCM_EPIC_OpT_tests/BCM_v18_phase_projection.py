# -*- coding: utf-8 -*-
"""
BCM v18 -- Phase-Projected Fractal Dissipation
=================================================
Stephen Justin Burdick Sr., 2026 -- Emerald Entities LLC
GIBUSH Systems

ChatGPT correction: "You are draining the wrong variable
in the wrong place."
...
Previous attempts drained sigma through Gamma. The debt
lives in phi. Fix: project phi onto continuous probe
density, dissipate grad_phi through that projection.

dE_bruce/dt = integral(grad_phi^2) dx
            - kappa * integral_Gamma(grad_phi^2) d_mu_Df

THREE RUNS:
  A: No dissipation (baseline)
  B: Chi tank (v17 method)
  C: Phase-projected fractal dissipation

Usage:
    python BCM_v18_phase_projection.py
...
class PhaseProbe:
    ...
    def update(self, com, pa, pb, step, sigma, nx, ny, rng,
        ...
        if self.cycle_step < b1:
            ...
            # Accumulate probe hit density (continuous)
...
def run_config(nx, ny, steps, dt, D, alpha, separation,
    ...
    # Phase field: accumulated difference between sigma
    # and its memory (the Brucetron lives here)
    phi = np.zeros((nx, ny))

    # Probe hit accumulator (continuous density, not binary)
    ...
    for step in range(steps):
        ...
        # BUILD PHASE FIELD: accumulated phase error
        # phi tracks the difference between current sigma
        # and what memory "expected" (sigma_prev)
        # This is where the Brucetron debt accumulates
        phase_error = sigma - sigma_prev
        phi = phi * 0.95 + phase_error  # decay + inject
        ...
        if mode == 'chi_tank':
            ...
            # PHASE-PROJECTED FRACTAL DISSIPATION
            # 1. Build continuous probe density (blurred hits)
            density = gaussian_blur(probe_hits, sigma_blur=3.0)
            d_max = float(np.max(density))
            if d_max > 0:
                density /= d_max  # normalize 0-1

            # 2. Compute grad_phi (phase field gradient)
            ...
            # 3. Fractal weighting: density^0.5 (D_f ~ 1.5)
            weight = np.power(density + 1e-10, 0.5)

            # 4. Dissipation: drain phi where grad_phi
            #    overlaps with probe density
            phi_dissip = kappa_phase * grad_phi_sq * weight
            phi -= phi_dissip
            # Also drain Brucetron field through same
            # projection
...
def main():
    parser = argparse.ArgumentParser(
        description="BCM v18 Phase-Projected Dissipation")
    ...
    print(f"\n{'='*65}")
    print(f"  BCM v18 -- PHASE-PROJECTED FRACTAL DISSIPATION")
    print(f"  Drain phi through probe density, not sigma")
    ...
    configs = [
        ...
        ("C: Phase projection", "phase_projection"),
    ]
    ...
    print(f"\n{'='*65}")
    print(f"  PHASE PROJECTION COMPARISON")
    print(f"{'='*65}")

    print(f"\n  {'Config':>28} {'Rate':>12} {'Late E':>10} "
          f"{'PhiRMS':>10} {'Dissip':>10}")
    ...
    print(f"    Phase projection (C):  {red_c:.1f}%")

    if rate_b != 0:
        c_vs_b = (1 - rate_c / rate_b) * 100
        print(f"    Projection vs chi:     {c_vs_b:.1f}%")

    print(f"\n{'='*65}")
    print(f"  PHASE PROJECTION VERDICT")
    print(f"{'='*65}")

    if rate_c < rate_b and rate_c < rate_a:
        if rate_c < 0:
            print(f"\n  PHASE PROJECTION ACHIEVES NEGATIVE GROWTH")
            print(f"  phi-space debt drained through probe density.")
            print(f"  The Frastrate closure is confirmed.")
        else:
            print(f"\n  PHASE PROJECTION OUTPERFORMS CHI TANK")
            print(f"  Draining the correct variable works.")
    elif rate_c < rate_a:
        print(f"\n  PHASE PROJECTION REDUCES GROWTH")
        if red_c > 30:
            print(f"  Significant coupling detected ({red_c:.1f}%).")
            print(f"  phi -> probe density projection is active.")
        else:
            print(f"  Partial coupling ({red_c:.1f}%).")
    else:
        print(f"\n  PHASE PROJECTION INEFFECTIVE")
        print(f"  phi-space projection not coupled.")
    ...
    out_path = os.path.join(out_dir,
        f"BCM_v18_phase_projection_"
        f"{time.strftime('%Y%m%d_%H%M%S')}.json")

    out_data = {
        "title": "BCM v18 Phase-Projected Fractal Dissipation",
        "author": ("Stephen Justin Burdick Sr. -- "
                    "Emerald Entities LLC"),
        "purpose": ("Drain phi (phase debt) through continuous "
                     "probe density, not sigma through binary mask"),
