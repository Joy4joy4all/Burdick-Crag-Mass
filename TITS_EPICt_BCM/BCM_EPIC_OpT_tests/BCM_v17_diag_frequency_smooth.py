...
def main():
    ...
    print(f"\n{'='*65}")
    print(f"  BCM v17 -- COHERENCE FREQUENCY (SMOOTHED MEMORY)")
    print(f"  Fractional memory delay kills f/2 subharmonic")
    ...
    sigma_prev2 = sigma.copy()  # 2-step history for smoothing
    ...
    for step in range(args.steps):
        ...
        if alpha > 0:
            # Smoothed memory: 75/25 blend removes sharp
            # 1-step reflection while preserving coherence
            mem_smooth = 0.75 * sigma_prev + 0.25 * sigma_prev2
            sn += alpha * (sigma - mem_smooth)
        sn = np.maximum(sn, 0)
        if float(np.max(sn)) > 1e10:
            break
        sigma_prev2 = sigma_prev.copy()
        sigma_prev = sigma.copy()
        sigma = sn
    ...
    f_memory = 0.5  # smoothed: 2-step average
