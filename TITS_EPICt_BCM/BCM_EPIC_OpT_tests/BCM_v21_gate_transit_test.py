...
def get_stellar_profile(step, total_steps):
    """
    INSTANT transit through Alpha Centauri A.
    At 12,000c you don't transit. You arrive.
    The star is a single flash. One frame.
    Settling after arrival is the only real phase.

    Returns: (lambda, stellar_pump_amplitude, phase_name)
    """
    # The star is at the midpoint. At 12,000c the entire
    # encounter is ~5 steps. You arrive, flash, settle.
    ...
    if step < center - 2:
        ...
        # Settling / recovery — the only real phase
...
def get_bh_profile(step, total_steps):
    ...
    3.8 Msun, 24 km. Master substrate pump.
    At 12,000c: 6.7 nanosecond crossing. 3 steps.
    """
    center = total_steps // 2
    if step < center - 1:
        return 0.05, 0.0, "APPROACH"
...
def main():
    ...
    parser.add_argument("--target", type=str, default="star",
                        choices=["star", "bh"],
                        help="star=Alpha Centauri A, bh=XTE J1650-500")
    args = parser.parse_args()
    nx=ny=args.grid; steps=args.steps
    target = args.target
    profile_fn = (get_bh_profile if target == "bh"
                  else get_stellar_profile)
    target_name = ("XTE J1650-500 (3.8 Msun, 24 km)"
                   if target == "bh"
                   else "Alpha Centauri A (m=1 tachocline)")
    settle_window = 1000 if target == "bh" else 600

    print(f"\n{'='*65}")
    print(f"  BCM v21 -- COHERENCE GATE TRANSIT TEST")
    print(f"  9D to 10D gate integrated into transit solver")
