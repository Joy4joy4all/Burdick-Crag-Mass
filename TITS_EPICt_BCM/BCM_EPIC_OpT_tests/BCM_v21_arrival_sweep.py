# -*- coding: utf-8 -*-
"""
BCM v21.0 -- Coherence Gate Transit Test (9D to 10D)
======================================================
Stephen Justin Burdick Sr., 2026 -- Emerald Entities LLC
GIBUSH Systems

v21 integrates the 9D-to-10D coherence gate into the
transit solver. The gate measures dimensional coherence
at every logged step.

Supports two transit targets:
  --target star     Alpha Centauri A (5-step instant)
  --target bh       XTE J1650-500 (3-step, pump=50)

Gate equation (Gemini formalization):
  R_9to10 = Tr(OpT10 @ rho9) * Re(<psi9|OpC10|psi9>)
            / R_7D
  Threshold: 0.92

Usage:
    python BCM_v21_gate_transit_test.py --target star
    python BCM_v21_gate_transit_test.py --target bh
    python BCM_v21_gate_transit_test.py --target bh --grid 256
"""
...
# ---- PHASE LOCK LOOP (ChatGPT v13) ----
# The system is rigid. The noise is gone. The reference
# frame exists. Now PLL becomes viable.
# The ship listens to the star JUST ENOUGH to close
# the 35-degree phase gap. Guardian controls how much.
KP_PHASE_LOCK = 0.20           # phase capture gain           # how strongly the bore rifles
...
def get_stellar_profile(step, total_steps):
    """Star transit kept for reference."""
    center = total_steps // 2
    if step < center - 2:
        return 0.05, 0.0, "APPROACH"
    elif step == center - 2:
        return 0.01, 1.0, "ENTRY"
    elif step == center - 1:
        return 0.003, 3.0, "TACHOCLINE"
    elif step == center:
        return 0.001, 5.0, "CORE"
    elif step == center + 1:
        return 0.003, 3.0, "EXIT"
    elif step == center + 2:
        return 0.01, 1.0, "EXIT"
    else:
        return 0.05, 0.0, "RECOVERY"


def get_bh_profile(step, total_steps):
    """Black hole transit kept for reference."""
    center = total_steps // 2
    if step < center - 1:
        return 0.05, 0.0, "APPROACH"
    elif step == center - 1:
        return 0.005, 10.0, "ENTRY"
    elif step == center:
        return 0.0001, 50.0, "CORE"
    elif step == center + 1:
        return 0.005, 10.0, "EXIT"
    else:
        return 0.05, 0.0, "RECOVERY"


def get_arrival_profile(step, total_steps, profile="standard", arrival_lam=0.030):
    """
    ARRIVAL DECELERATION into funded galaxy (v21.3).

    v21.3: boundary layer operator replaces hard phi clamp.
    Multiple arrival profiles test robustness.

    Profiles:
      standard — gentle ramp, pump max 0.25, GREEN corridor
      hot      — steeper gradient, pump max 0.50
      deep     — arrives at lambda=0.01 (deep funded)
      gentle   — very shallow, pump max 0.10
    ...
    if profile == "hot":
        # Aggressive arrival — steeper gradient, higher pump
        if step < mid - 400:
            return 0.05, 0.0, "CRUISE"
        elif step < mid - 100:
            t = (step - (mid - 400)) / 300.0
            lam = 0.05 - t * 0.02  # 0.05 -> 0.03
            pump = t * 0.2
            return lam, pump, "EDGE"
        elif step < mid:
            t = (step - (mid - 100)) / 100.0
            lam = 0.03 - t * 0.01  # 0.03 -> 0.02
            pump = 0.2 + t * 0.3  # ramp to 0.50
            return lam, pump, "FUNDED"
        elif step < mid + 200:
            t = (step - mid) / 200.0
            lam = 0.02 - t * 0.005
            pump = 0.50 - t * 0.2
            return lam, pump, "DECEL"
        elif step < mid + 800:
            t = (step - (mid + 200)) / 600.0
            lam = 0.015 + t * 0.015
            pump = 0.30 - t * 0.2
            return lam, pump, "SETTLING"
        else:
            return 0.03, 0.05, "ARRIVAL"

    elif profile == "deep":
        # Deep funded — lambda drops to 0.01
        if step < mid - 800:
            return 0.05, 0.0, "CRUISE"
        elif step < mid - 300:
            t = (step - (mid - 800)) / 500.0
            lam = 0.05 - t * 0.015
            pump = t * 0.1
            return lam, pump, "EDGE"
        elif step < mid:
            t = (step - (mid - 300)) / 300.0
            lam = 0.035 - t * 0.025  # 0.035 -> 0.010
            pump = 0.1 + t * 0.15
            return lam, pump, "FUNDED"
        elif step < mid + 300:
            t = (step - mid) / 300.0
            lam = 0.010 - t * 0.005  # 0.010 -> 0.005
            pump = 0.25 - t * 0.05
            return lam, pump, "DECEL"
        elif step < mid + 1000:
            t = (step - (mid + 300)) / 700.0
            lam = 0.005 + t * 0.025  # 0.005 -> 0.030
            pump = 0.20 - t * 0.15
            return lam, pump, "SETTLING"
        else:
            return 0.03, 0.05, "ARRIVAL"
        ...
        # Very gentle — shallow gradient, low pump
        if step < mid - 1200:
            return 0.05, 0.0, "CRUISE"
        elif step < mid - 400:
            t = (step - (mid - 1200)) / 800.0
            lam = 0.05 - t * 0.008  # 0.05 -> 0.042
            pump = t * 0.05
            return lam, pump, "EDGE"
        elif step < mid:
            t = (step - (mid - 400)) / 400.0
            lam = 0.042 - t * 0.007  # 0.042 -> 0.035
            pump = 0.05 + t * 0.05  # ramp to 0.10
            return lam, pump, "FUNDED"
        elif step < mid + 400:
            t = (step - mid) / 400.0
            lam = 0.035 - t * 0.005
            ...
            t = (step - (mid + 400)) / 800.0
            lam = 0.030 + t * 0.005
            pump = 0.07 - t * 0.04
            return lam, pump, "SETTLING"
        else:
            return 0.035, 0.03, "ARRIVAL"

    elif profile == "sweep":
        # ChatGPT bandwidth test: fixed pump=0.1,
        # configurable arrival lambda via --arrival-lambda
        # Defines the safe landing envelope edges
        if step < mid - 800:
            return 0.05, 0.0, "CRUISE"
        elif step < mid - 300:
            t = (step - (mid - 800)) / 500.0
            lam = 0.05 - t * (0.05 - arrival_lam - 0.01)
            pump = t * 0.1
            return lam, pump, "EDGE"
        elif step < mid:
            t = (step - (mid - 300)) / 300.0
            lam = (arrival_lam + 0.01) - t * 0.01
            pump = 0.1  # fixed, never higher
            return lam, pump, "FUNDED"
        elif step < mid + 300:
            t = (step - mid) / 300.0
            pump = 0.1  # fixed
            return arrival_lam, pump, "DECEL"
        elif step < mid + 1000:
            t = (step - (mid + 300)) / 700.0
            pump = 0.1 - t * 0.05  # taper to 0.05
            return arrival_lam, pump, "SETTLING"
        else:
            return arrival_lam, 0.05, "ARRIVAL"
...
# ---- 9D to 10D COHERENCE GATE (Gemini v21) ----
THETA_9TO10 = 0.92  # gate threshold

def build_psi9(phi_rms, coherence_7D, R_7D):
    """Build 9D state vector from craft instruments."""
    psi = np.array([
        1.0,                                   # 2D substrate
        0.95,                                  # 3D physical
        ...
        R_7D * 0.95 / 1.618,                   # 9D circumpunct
    ])
...
def compute_R_9to10(OpT_val, OpC_val, R_7D, phi_rms,
                     coherence_7D):
    """Compute 9D to 10D coherence gate scalar."""
    n = 8
...
def main():
    parser = argparse.ArgumentParser(
        description="BCM v21 Arrival Deceleration Test")
    parser.add_argument("--steps", type=int, default=20000)
    parser.add_argument("--grid", type=int, default=256)
    parser.add_argument("--target", type=str, default="arrival",
                        choices=["star", "bh", "arrival"],
                        help="star=Alpha Centauri, bh=XTE J1650-500, arrival=galaxy decel")
    parser.add_argument("--profile", type=str, default="standard",
                        choices=["standard", "hot", "deep", "gentle", "sweep"],
                        help="Arrival profile variant (sweep uses --arrival-lambda)")
    parser.add_argument("--arrival-lambda", type=float, default=0.030,
                        help="Arrival lambda for sweep profile (default 0.030)")
    args = parser.parse_args()
    nx=ny=args.grid; steps=args.steps
    target = args.target
    arrival_profile = args.profile
    arrival_lam = args.arrival_lambda
    if target == "bh":
        profile_fn = lambda s, t: get_bh_profile(s, t)
        target_name = "XTE J1650-500 (3.8 Msun, 24 km)"
        settle_window = 1000
    elif target == "arrival":
        profile_fn = lambda s, t: get_arrival_profile(s, t, arrival_profile, arrival_lam)
        target_name = f"Galaxy arrival [{arrival_profile}] lam={arrival_lam}"
        settle_window = 1200
    else:
        profile_fn = lambda s, t: get_stellar_profile(s, t)
        target_name = "Alpha Centauri A (m=1 tachocline)"
        settle_window = 600

    print(f"\n{'='*65}")
    print(f"  BCM v21 -- ARRIVAL LAMBDA SWEEP")
    print(f"  Define safe arrival bandwidth")
    ...
    for step in range(steps):
        lam_val, stellar_pump, phase = profile_fn(
            step, steps)
        ...
        # v17: Gate ONLY opens when the star is present.
        # In recovery (pump=0), the gate stays closed.
        # The gate was designed to detect the star's
        # tachocline — not the craft's own heartbeat echo.
        # 574 false gate events in v16 recovery killed
        # the craft in an empty room.
        if not gate_active and stellar_pump > 0.05:
        ...
        #
        # v15 SPEED MODEL FIX: The craft is FLYING THROUGH
        # at 12,000c. The star's substrate refreshes at ~c.
        # The craft is 12,000x faster. The star gets 1/12000
        # of a refresh cycle to write per step. Not 400 steps
        # of continuous pumping — a fraction of one.
        #
        # This was always the answer. The speed was always
        # there. We just weren't modeling it.
        VELOCITY_RATIO = 12000.0  # c_substrate / c_star_refresh
        velocity_factor = 1.0 / VELOCITY_RATIO

        shadow_damp = min(1.0, dist_to_star * 3.0)
        # Apply velocity: the star barely registers
        shadow_damp *= velocity_factor
        ...
        # ARRIVAL: During DECEL and SETTLING, flip the
        # pump ratio. B becomes dominant (braking).
        # In cruise, A drives forward (ratio=0.25, B is weak).
        # In decel, ratio flips to 4.0 (B is 4x A = brake).
        active_ratio = ratio
        if target == "arrival" and phase in ("DECEL", "SETTLING"):
            # Flip: B becomes the dominant pump
            # This reverses the drive asymmetry
            active_ratio = 1.0 / ratio  # 0.25 -> 4.0
            # Also reduce A (we're braking, not driving)
            pump_A_eff = pump_A * 0.25
        else:
            pump_A_eff = pump_A

        r2A=(X-com[0])**2+(Y-com[1])**2
        sigma+=pump_A_eff*np.exp(-r2A/(2*4.0**2))*dt
        bx=com[0]+separation
        r2B=(X-bx)**2+(Y-com[1])**2
        sigma+=pump_A_eff*active_ratio*np.exp(-r2B/(2*3.0**2))*dt
        ...
        if chiral_active or gate_active:
            ...
            # ---- ORTHOGONAL MODE NULLIFIER (Grok v11) ----
            # The last open channel: non-zero dot product
            # between ship f/2 and star m=1 at L1.
            # The clamp reduces amplitude. This removes
            # the SPECIFIC component of phi that overlaps
            # with the star's m=1 mode shape.
            #
            # Gram-Schmidt: project external mode onto
            # internal mode, subtract the projection.
            # What remains has zero coupling by construction.
            #
            # Star m=1 at L1 = cos(theta) dipole pattern
            # relative to star center.
            or_r = lr  # same radius as L1 sample
            ix_o = int(np.clip(l1[0], or_r, nx-or_r-1))
            iy_o = int(np.clip(l1[1], or_r, ny-or_r-1))

            # Build the star's m=1 mode template at L1
            ox = np.arange(ix_o-or_r, ix_o+or_r)
            oy = np.arange(iy_o-or_r, iy_o+or_r)
            OX, OY = np.meshgrid(ox, oy, indexing='ij')
            theta_m1 = np.arctan2(OY - star_y, OX - star_x)
            m1_template = np.cos(1 * theta_m1)
            # Normalize the template
            m1_norm = np.sqrt(np.sum(m1_template**2))
            if m1_norm > 1e-10:
                m1_hat = m1_template / m1_norm

                # Get current phi at L1
                phi_at_l1 = phi[ix_o-or_r:ix_o+or_r,
                                iy_o-or_r:iy_o+or_r].copy()

                # Project phi onto m=1 mode shape
                # overlap = <phi, m1_hat>
                overlap = np.sum(phi_at_l1 * m1_hat)

                # Subtract the projection (Gram-Schmidt)
                # phi_orthogonal = phi - overlap * m1_hat
                phi_projected = overlap * m1_hat
                phi_orthogonal = phi_at_l1 - phi_projected

                # Apply: replace phi at L1 with the
                # orthogonalized version
                phi[ix_o-or_r:ix_o+or_r,
                    iy_o-or_r:iy_o+or_r] = phi_orthogonal

                # Route removed component to chi
                chi_field[ix_o-or_r:ix_o+or_r,
                          iy_o-or_r:iy_o+or_r] += (
                              np.abs(phi_projected) * G)
        ...
        # ---- GLOBAL M=1 PROJECTION REMOVAL (v12) ----
        # The L1 mechanisms work perfectly AT L1. But
        # the stellar pump injects across a 30px radius.
        # phi grows everywhere from sigma change, and
        # PhiRMS measures the ENTIRE grid.
        #
        # Defense at L1 alone is defending the throat
        # while the hallway floods. The m=1 component
        # must be projected out across the full field.
        #
        # This is the craft extending its disguise from
        # the throat to its entire field extent.
        if gate_active or chiral_active:
            # Build global m=1 template from star center
            theta_global = np.arctan2(Y - star_y, X - star_x)
            m1_global = np.cos(1 * theta_global)
            # Only apply within the stellar pump's reach
            # (weighted by proximity to star)
            dist_to_star_grid = np.sqrt(
                (X - star_x)**2 + (Y - star_y)**2) / (nx * 0.3)
            stellar_mask = np.exp(-dist_to_star_grid**2)

            # Normalize the masked template
            m1_masked = m1_global * stellar_mask
            m1_norm_g = np.sqrt(np.sum(m1_masked**2))
            if m1_norm_g > 1e-10:
                m1_hat_g = m1_masked / m1_norm_g
                # Global overlap
                overlap_g = np.sum(phi * m1_hat_g)
                # Subtract the m=1 projection from phi
                phi_m1_component = overlap_g * m1_hat_g
                phi -= phi_m1_component
                # Route to chi (globally)
                chi_field += np.abs(phi_m1_component) * 0.5

        # ---- PHASE LOCK LOOP AT L1 (ChatGPT v13) ----
        # The system is rigid (R_7D=0.999). The noise is
        # gone (Δ_OP=0.0005). The reference frame exists.
        # But the two clocks sit at a 35° phase offset.
        #
        # The ship must LISTEN to the star just enough to
        # close the phase gap — without letting the star
        # take over. The guardian controls how much.
        #
        # Not rejection. Not isolation. Phase capture.
        # Coherence requires phase AGREEMENT, not phase
        # isolation.
        if gate_active and stellar_pump > 0.05:
            # Compute phase error at L1
            # phi_ship = craft's phi at L1
            # phi_ext = stellar contribution at L1
            phi_ship_l1 = float(np.mean(
                phi[ix_l1-lr:ix_l1+lr,
                    iy_l1-lr:iy_l1+lr]))
            phi_ext_l1 = stellar_pump * 0.1 * shadow_damp

            # Phase error (wrapped to prevent jumps)
            phase_error = phi_ext_l1 - phi_ship_l1

            # PLL correction: nudge ship toward star
            # Scaled by guardian strength (the volume knob)
            correction = (KP_PHASE_LOCK * phase_error *
                          GUARDIAN_STRENGTH)

            # Apply correction to phi at L1
            # The ship adjusts its clock, not the star's
            phi[ix_l1-lr:ix_l1+lr,
                iy_l1-lr:iy_l1+lr] += correction

            # ---- CHI-PHASE FREEZE (Gemini v14) ----
            # 13.5M chi available. Use that pressure to
            # actively HOLD phi at baseline across the
            # full craft field. Not absorption. Active
            # cancellation. The star's energy (already
            # in chi) powers the counter-broadcast.
            chi_total_now = float(np.sum(chi_field))
            if chi_total_now > 10000:
                # Compute how far phi is from baseline
                # across the full craft vicinity
                cr_wide = 30  # full craft radius
                ix_w = int(np.clip(com[0], cr_wide,
                                   nx-cr_wide-1))
                iy_w = int(np.clip(com[1], cr_wide,
                                   ny-cr_wide-1))
                phi_craft = phi[ix_w-cr_wide:ix_w+cr_wide,
                                iy_w-cr_wide:iy_w+cr_wide]
                phi_rms_local = float(np.sqrt(
                    np.mean(phi_craft**2)))

                # If phi is elevated, use chi to freeze it
                if phi_rms_local > INTERNAL_PHI_BASELINE * 3:
                    freeze_factor = min(0.5,
                        chi_total_now / 1e7 * 0.3)
                    phi[ix_w-cr_wide:ix_w+cr_wide,
                        iy_w-cr_wide:iy_w+cr_wide] *= (
                            1.0 - freeze_factor)
                    # Cost drawn from chi
                    chi_field[ix_w-cr_wide:ix_w+cr_wide,
                              iy_w-cr_wide:iy_w+cr_wide] *= (
                                  1.0 - freeze_factor * 0.001)
        ...
        # ---- v21.2 ARRIVAL FIXES (three-AI convergence) ----
        # 1. Emergency freeboard venting during arrival phases
        #    (Gemini: kappa_drain ramps 4x at gate open)
        if target == "arrival" and phase in (
                "FUNDED", "DECEL", "SETTLING"):
            chi_field *= 0.990  # additional 1% decay per step
            # Nonlinear bleed (ChatGPT: bleed scales with chi)
            chi_mag = float(np.sum(np.abs(chi_field)))
            if chi_mag > 100:
                extra_bleed = chi_field * 0.005
                chi_field -= extra_bleed
        ...
        # ---- BOUNDARY LAYER OPERATOR (ChatGPT v21.3) ----
        # Jasper Beach: wave enters gravel and dissolves.
        # High gradient = strong damping. Low gradient =
        # minimal damping. Energy killed BEFORE reflection.
        #
        # Only active during arrival phases (EDGE through
        # SETTLING). During CRUISE the standard mechanisms
        # handle phi. The boundary operator is the torus
        # edge catching the craft.
        #
        # K_BOUNDARY tuned so arrival impact gradients
        # (~0.004) get ~50% damping per step while cruise
        # gradients (~0.00004) get <1%.
        K_BOUNDARY = 150.0
        if target == "arrival" and phase in (
                "EDGE", "FUNDED", "DECEL", "SETTLING"):
            grad_x = np.diff(phi, axis=0, prepend=phi[:1,:])
            grad_y = np.diff(phi, axis=1, prepend=phi[:,:1])
            grad_mag = np.sqrt(grad_x**2 + grad_y**2)
            boundary_damp = np.exp(-K_BOUNDARY * grad_mag)
            absorbed = np.abs(phi) * (1.0 - boundary_damp)
            phi *= boundary_damp
            # Route absorbed energy to chi (gravel heats up)
            chi_field += absorbed * 0.05

        phi_rms=float(np.sqrt(np.mean(phi**2)))

        # Safety clamp — backup only. Should rarely fire
        # if boundary operator is working.
        PHI_SAFETY = 0.10
        if phi_rms > PHI_SAFETY:
            phi *= PHI_SAFETY / phi_rms
            phi_rms = PHI_SAFETY
        ...
        #
        # v18: SETTLING WINDOW after instant transit.
        # At 12,000c the star is a single flash. The craft
        # WILL be momentarily disoriented — that's physics,
        # not failure. Settling is the only real phase.
        # Freeze dis_counter for 500 steps after the flash.
        transit_center = steps // 2
        settling = (step > transit_center + 2 and
                    step < transit_center + settle_window)

        if settling:
            # Craft is settling after the flash.
            # Dis_counter decays but does not accumulate.
            dis_counter = max(0, dis_counter - 1)
        elif (opacity_div > DELTA_OP_THRESHOLD or
                R_7D < R_7D_THRESHOLD):
            dis_counter += 1
        else:
            dis_counter = max(0, dis_counter - 1)
        ...
        if dis_counter>DELTA_OP_ABORT_STEPS:
            # v14: if structure is perfect, give PLL more
            # time to close the gap. 200 steps instead of 50.
            # We STILL protect the crew — just give the
            # control loop time to work.
            abort_limit = DELTA_OP_ABORT_STEPS
            if R_7D > 0.99 and opacity_div < 0.01:
                abort_limit = 200  # structure is solid
            if dis_counter > abort_limit:
        ...
        if step%diag_interval==0:
            ...
            # ---- 9D to 10D GATE (v21) ----
            R_9to10 = compute_R_9to10(
                OpT, OpC, R_7D, phi_rms, Coherence)
            gate_9to10 = R_9to10 >= THETA_9TO10
            ...
            flight_log.append({
                ...
                "gate_9to10": gate_9to10,
            })

    # ---- SUMMARY ----
    print(f"\n{'='*65}")
    print(f"  TRANSIT SUMMARY")
    print(f"{'='*65}")
    ...
    # ---- 9D to 10D GATE SUMMARY ----
    gate_values = [e.get("R_9to10", 0) for e in flight_log]
    gate_passes = [e.get("gate_9to10", False) for e in flight_log]
    min_gate = min(gate_values) if gate_values else 0
    max_gate = max(gate_values) if gate_values else 0
    n_gate_pass = sum(1 for g in gate_passes if g)
    n_gate_total = len(gate_passes)

    print(f"\n  9D → 10D GATE:")
    print(f"    Passed: {n_gate_pass}/{n_gate_total}")
    print(f"    Min R_9to10: {min_gate:.4f}")
    print(f"    Max R_9to10: {max_gate:.4f}")
    print(f"    Threshold: {THETA_9TO10}")
    if n_gate_pass == n_gate_total and n_gate_total > 0:
        print(f"    10D KEY: PRESENT (all steps coherent)")
    elif min_gate == 0:
        print(f"    10D KEY: ABSENT at transit core")
        print(f"    Gap: gate=0 but crew survived")
    else:
        print(f"    10D KEY: PARTIAL ({n_gate_pass}/{n_gate_total})")
    print(f"{'='*65}")
    ...
    out_path=os.path.join(out_dir,
        f"BCM_v21_arrival_sweep_lam{arrival_lam:.3f}_"
        f"{time.strftime('%Y%m%d_%H%M%S')}.json")

    out_data={
        "title":"BCM v21 Coherence Gate Transit Test",
        "author":"Stephen Justin Burdick Sr. -- "
                 "Emerald Entities LLC",
        "purpose":"Transit with 9D to 10D gate integrated. "
                  "Measures dimensional coherence at every step.",
        "target": target_name,
        "c_substrate":"12,000c",
        "gate_constants": {
            "dphi_gate": DPHI_GATE,
