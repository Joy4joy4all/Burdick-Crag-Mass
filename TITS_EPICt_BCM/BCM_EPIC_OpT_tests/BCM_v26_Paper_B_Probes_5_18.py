# -*- coding: utf-8 -*-
"""
BCM v26 Paper B Probe Test 5_18
Paper B Section 7.2 -- alpha-Regularization Perturbation
                       (Kinetic-Topological Decoupling Test)
...
Psi_bruce, the Kinetic-Topological Decoupling claim (advisor-
named Theorem III) that alpha is a kinetic parameter orthogonal
to the Taylor a-coefficient's topological effect, the alpha-
regularization perturbation design, the three-gate decoupling
test structure (K1 hysteresis response, K2 topological
invariance, K3 baseline anchor), and every originating insight
in this file belong solely to Stephen Justin Burdick Sr. Stephen
Burdick is the sole discoverer and theoretical originator of the
BCM framework. Four AI systems (Claude, ChatGPT, Gemini, Grok)
assist the project as computational tools and reviewers at the
direction of the Foreman. No AI system owns or co-owns any
theoretical concept. Emerald Entities LLC -- GIBUSH Systems.

--------------------------------------------------------------------
CONTEXT
--------------------------------------------------------------------
5_14 / 5_15 / 5_16 established that the four-field state tensor is
xi-invariant across [0.005, 0.07]. 5_17 broke the invariance
along the Taylor a-coefficient axis, showing lambda_c scales
~linearly with a via the relation lambda_c = kappa_spectral *
(a/b) with kappa_spectral ~ 0.0592 from empirical data.

The open question after 5_17 is whether alpha (the memory
regularization coefficient in the PDE update sigma_new =
sigma + dt*rhs + alpha*(sigma - sigma_prev)) is:

  (a) a kinetic-only axis that modulates hysteresis but preserves
      topology (advisor-framed Theorem III: Kinetic-Topological
      Decoupling), OR

  (b) a topological axis that also shifts lambda_c and the other
      spectral fields.

Test 5_18 answers this directly.

--------------------------------------------------------------------
HONEST PRE-RUN FLAG
--------------------------------------------------------------------
Three possible outcomes for H(alpha), all physically defensible:

  Outcome 1: H(alpha=0) -> 0. Memory is the sole hysteresis
             mechanism. alpha is a clean kinetic control axis.

  Outcome 2: H(alpha=0) stays near 0.05 (5_15 baseline). 5_14
             audit established that J is numerically inactive
             under the current Psi_bruce formulation. A dead
             field in the cold torus has no mechanism to restart
             on the return sweep regardless of alpha. Hysteresis
             would then be driven by absorbing-state asymmetry,
             not memory. alpha would still affect transition
             dynamics (overshoot / convergence rate) but not H
             itself.

  Outcome 3: H(alpha=0) > baseline. Memory is actively
             suppressing hysteresis by sustaining the field
             through the transition. Removing it would sharpen
             the collapse. Least expected.

This file does not commit to any outcome. It records the data
and reports what the gates say. For Paper B Section 5.2, the
actual mechanism must be drawn from 5_18 results, not from prior
hypotheses.

--------------------------------------------------------------------
SCOPE
--------------------------------------------------------------------
alpha values: [0.0, 0.2, 0.5, 0.8, 0.95]

  alpha = 0.0  pure forward Euler, no momentum
  alpha = 0.2  light momentum
  alpha = 0.5  moderate momentum
  alpha = 0.8  5_15 baseline
  alpha = 0.95 strong momentum (near instability boundary)

All other parameters frozen at 5_15 baseline: a = 0.12, b = 0.12,
xi = 0.035 (numerically inactive), pumps from 5_7, grid 128,
sweep [0.001, 0.30] 30 log-spaced dual, classifier v2.

For each alpha, extract the 4-field tensor:

  lambda_c        (active-region sigmoid inflection)
  lambda_fold     (argmax n_steps in active region, stiffness centroid)
  lambda_cold     (first Phi-dead-stayed lambda)
  H(alpha)        (restricted hysteresis area)

Three gates:

  K1 alpha modulates H: |Pearson(alpha, H)| >= 0.80 OR
     relative range of H across alpha >= 0.30. Either condition
     sufficient. Tests whether alpha affects hysteresis at all.

  K2 alpha preserves topology: relative range of lambda_c across
     alpha < 0.01 AND same for lambda_fold AND lambda_cold.
     Tests Kinetic-Topological Decoupling. If K2 fails, alpha
     has topological effect and the decoupling framing breaks.

  K3 baseline anchor: at alpha = 0.8, lambda_c reproduces the
     5_15 value 0.059165791 within 1e-6. Solver reproducibility.

PASS on K1 AND K2 AND K3 registers
H_PAPER_B_ALPHA_KINETIC_DECOUPLING with primary evidence.
"""
...
# Five alpha values (Foreman-specified)
ALPHA_VALUES = [0.0, 0.2, 0.5, 0.8, 0.95]

# Baseline anchor values from 5_15 / 5_16 at alpha = 0.8
...
CFG = {
    # PDE (frozen at 5_15 baseline except alpha which varies per iter)
    "dt": 0.05,
    "D_diffusion": 0.5,
    "alpha_regularization": 0.8,     # per-iteration override
    "ham_s_coeff_a": 0.12,
    "ham_s_coeff_b": 0.12,
    "xi_coupling": 0.035,            # inactive per 5_14 audit
    ...
    # Kinetic-topological decoupling gate thresholds
    "k1_pearson_min_abs": 0.80,
    "k1_rel_range_min": 0.30,
    "k2_topology_rel_range_max": 0.01,
    "k3_baseline_tolerance": 1e-6,
}


# =====================================================================
# KERNEL PRIMITIVES (identical to 5_17)
# =====================================================================
...
def kernel_validation(cfg):
    ...
    return {
        "alpha_at_validation": cfg["alpha_regularization"],
...
def run_single_alpha(alpha_value, base_cfg):
    cfg = dict(base_cfg)
    cfg["alpha_regularization"] = float(alpha_value)
    ...
    # Forward-sweep convergence totals (kinetic signature)
...
def evaluate_decoupling_gates(tensor, cfg):
    alpha_vals = [t["alpha"] for t in tensor]
    h_vals = [t["H_alpha"] for t in tensor]
    ...
    # K1: alpha modulates H (either correlation OR range threshold)
    h_corr = pearson_correlation(alpha_vals, h_vals)
    h_range = relative_range(h_vals)
    corr_pass = (h_corr is not None and abs(h_corr) >= cfg["k1_pearson_min_abs"])
    range_pass = (h_range is not None
                  and h_range["relative_range"] >= cfg["k1_rel_range_min"])
    k1_pass = corr_pass or range_pass

    # K2: topology invariant under alpha
    ...
    tol = cfg["k2_topology_rel_range_max"]
    ...
    k2_lambda_c_pass = inv_pass(rr_lambda_c)
    k2_lambda_fold_pass = inv_pass(rr_lambda_fold)
    k2_lambda_cold_pass = inv_pass(rr_lambda_cold)
    k2_pass = k2_lambda_c_pass and k2_lambda_fold_pass and k2_lambda_cold_pass

    # K3: baseline reproducibility at alpha = 0.8
    baseline_point = next(
        (t for t in tensor if abs(t["alpha"] - 0.8) < 1e-9), None
    )
    k3_pass = False
    k3_deviation = None
    if baseline_point is not None and baseline_point["lambda_c_spectral"] is not None:
        k3_deviation = abs(baseline_point["lambda_c_spectral"] - BASELINE_LAMBDA_C)
        k3_pass = bool(k3_deviation < cfg["k3_baseline_tolerance"])

    gates = {
        "K1_alpha_modulates_H": {
            "passed": k1_pass,
            "pearson_correlation": h_corr,
            "pearson_threshold": cfg["k1_pearson_min_abs"],
            "corr_pass": corr_pass,
            "relative_range": h_range,
            "range_threshold": cfg["k1_rel_range_min"],
            "range_pass": range_pass,
        },
        "K2_topology_invariant": {
            "passed": k2_pass,
            "tolerance": tol,
            "lambda_c_invariant": k2_lambda_c_pass,
            "lambda_c_relative_range": rr_lambda_c,
            "lambda_fold_invariant": k2_lambda_fold_pass,
            "lambda_fold_relative_range": rr_lambda_fold,
            "lambda_cold_invariant": k2_lambda_cold_pass,
            "lambda_cold_relative_range": rr_lambda_cold,
        },
        "K3_baseline_anchor": {
            "passed": k3_pass,
            "baseline_lambda_c_5_15": BASELINE_LAMBDA_C,
            "measured_lambda_c_alpha_0_8": (
                baseline_point["lambda_c_spectral"] if baseline_point else None
            ),
            "deviation": k3_deviation,
            "tolerance": cfg["k3_baseline_tolerance"],
        },
    }
    ...
    return gates, result, passed_count
...
def main():
    print("=" * 76)
    print("BCM v26 Paper B Probe Test 5_18 -- alpha-Regularization Perturbation")
    ...
    print(f"Objective       = test Kinetic-Topological Decoupling")
    print(f"                  (alpha modulates H but preserves topology?)")
    print()
    ...
    for idx, alpha_val in enumerate(ALPHA_VALUES):
        print(f"[{idx + 1}/{len(ALPHA_VALUES)}] alpha = {alpha_val} ...")
        point, payload = run_single_alpha(alpha_val, CFG)
        tensor.append(point)
        alpha_payloads[f"alpha_{alpha_val}"] = payload
    ...
    print("=" * 76)
    print("ALPHA PERTURBATION TENSOR")
    print("=" * 76)
    header = (f"{'alpha':>6}  {'lambda_c':>12}  {'lambda_fold':>12}  "
              f"{'lambda_cold':>12}  {'H(alpha)':>10}  {'R^2':>7}  "
              f"{'fwd_steps':>10}")
    print(header)
    print("-" * len(header))
    for t in tensor:
    ...
    out_path = resolve_output_path(f"BCM_v26_Paper_B_Probes_5_18_{stamp}.json")

    hypotheses_tested = {
        "H_PAPER_B_ALPHA_KINETIC_DECOUPLING": {
            "statement": (
                "H-PaperB-AlphaKinetic The alpha-regularization "
                "coefficient is a kinetic-only control axis: it "
                "modulates the hysteresis area H (K1: "
                "|Pearson(alpha, H)| >= 0.80 or relative range "
                ">= 30%) while preserving the topological fields "
                "lambda_c, lambda_fold_center, lambda_cold_entry "
                "(K2: relative range < 1% across alpha in [0.0, "
                "0.95]). The baseline at alpha = 0.8 reproduces "
                "the 5_15 lambda_c = 0.059165791 within 1e-6 "
                "(K3). PASS establishes the Kinetic-Topological "
                "Decoupling claim (advisor-framed Theorem III): "
                "alpha and the Taylor a-coefficient operate on "
                "orthogonal fields of the four-field state tensor."
            ),
            "result": result,
    ...
    payload = {
        "test_name": "BCM_v26_Paper_B_Probes_5_18",
        "test_number": 5,
        "test_sub_number": 18,
        "test_version": 1,
        ...
        "paper_b_section": "7.2",
        "paper_b_probe": "alpha-regularization kinetic-topological decoupling",
        "test_role": "primary_measurement",
        "grid": f"{CFG['grid_N']}x{CFG['grid_N']}",
        "notes": (
            "Test 5_18 varies alpha across [0.0, 0.2, 0.5, 0.8, "
            "0.95] with all other parameters frozen at 5_15 "
            "baseline. Extracts four-field tensor per alpha and "
            "tests three gates: K1 alpha modulates H, K2 alpha "
            "preserves topology, K3 baseline anchor at alpha = "
            "0.8 reproduces 5_15. PASS on all three confirms "
            "Kinetic-Topological Decoupling (advisor's Theorem "
            "III). The test also records forward and return "
            "total steps per alpha as a kinetic signature -- "
            "alpha near 0 is expected to converge differently "
            "than alpha near 0.95 regardless of topology."
        ),
        ...
        "gate_thresholds": {
            "k1_pearson_min_abs": CFG["k1_pearson_min_abs"],
            "k1_rel_range_min": CFG["k1_rel_range_min"],
            "k2_topology_rel_range_max": CFG["k2_topology_rel_range_max"],
            "k3_baseline_tolerance": CFG["k3_baseline_tolerance"],
        },
        "alpha_perturbation_tensor": tensor,
        ...
        "per_alpha_payload": alpha_payloads,
        "hypotheses_tested": hypotheses_tested,
        "scope": (
            "Five-value alpha sweep with full dual-sweep per "
            "alpha and four-field tensor extraction. Compared "
            "against the 5_15 / 5_16 baseline at alpha = 0.8. "
            "Tests the Kinetic-Topological Decoupling claim: "
            "that alpha and the Taylor a-coefficient operate on "
            "orthogonal fields of the state tensor. Combined "
            "with 5_17 a-coefficient perturbation, this probes "
            "a two-axis operator map (a controls topology, alpha "
            "controls hysteresis) with xi as a null axis."
        ),
        "next_tests_reserved": {
            "5_19": "D-diffusion scan (spatial coupling strength)",
            "5_20": "b-coefficient scan (saturation strength; pair with a)",
        },
        "keywords": [
            "paper_b",
            "section_7_2",
        ...
        "primacy_statement": (
            ...
            "Brucetron and Psi_bruce, the Kinetic-Topological "
            "Decoupling claim, the alpha-perturbation design, "
            "the three-gate structure (K1 hysteresis response, "
            "K2 topological invariance, K3 baseline anchor), "
            "and every originating insight belong solely to "
            "Stephen Justin Burdick Sr. Four AI systems assist "
            "the project at the direction of the Foreman; no AI "
            "system owns or co-owns any theoretical concept. "
