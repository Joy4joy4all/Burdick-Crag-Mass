...
def get_stellar_profile(step, total_steps):
    """
    INSTANT transit through XTE J1650-500.
    Black hole: 3.8 M_sun, 24 km diameter.
    The master substrate pump of its system.

    At 12,000c the craft crosses 24 km in 6.7 nanoseconds.
    The black hole is not a frame. It is a pixel.
    But the pump is the strongest thing in the galaxy.

    Alpha Centauri A was 5 steps. This is 1 step.
    The star was pump=5.0. This is pump=50.0.
    The star was the training run. This is the target.
    ...
    if step < center - 1:
        ...
        # Entry gradient — lambda drops to near zero
        return 0.005, 10.0, "ENTRY"
    elif step == center:
        # THE FLASH — master pump, 1 step
        # Lambda near zero: maximally funded substrate
        # Pump=50: the strongest oscillator in the system
        return 0.0001, 50.0, "CORE"
    elif step == center + 1:
        # Exit gradient
        return 0.005, 10.0, "EXIT"
    else:
        # Settling — the only real phase
...
def main():
    ...
    print(f"\n{'='*65}")
    print(f"  BCM v20 -- BLACK HOLE TRANSIT TEST")
    print(f"  Fly through XTE J1650-500.")
    print(f"  3.8 Msun, 24 km. Master substrate pump.")
    print(f"  The star was the training run.")
    ...
    print(f"  Target: XTE J1650-500 (3.8 Msun, 24 km)")
    ...
    for step in range(steps):
        ...
        # ---- MIRROR POLISH (v19 — settling phase) ----
        # The mirror fogs 200-400 steps AFTER the flash,
        # not during it. The phi shockwave arrives late.
        # During settling, actively restore OpT/OpC toward
        # equilibrium. The guardians reinforce the polish.
        transit_mid = steps // 2
        in_settling = (step > transit_mid and
                       step < transit_mid + 1000)
        if in_settling and opacity_div > 0.005:
            # Polish factor: stronger when divergence is
            # higher, scaled by guardian strength
            polish = min(0.95, opacity_div * 2.5)
            # Drive both operators toward their mean
            op_mean = (OpT + OpC) / 2.0
            OpT = OpT + (op_mean - OpT) * polish * GUARDIAN_STRENGTH
            OpC = OpC + (op_mean - OpC) * polish * GUARDIAN_STRENGTH
            # Recompute after polish
            opacity_div = abs(OpT - OpC)
        ...
        if chiral_active or gate_active:
            ...
            # ---- GRADIENT KILL AT L1 (ChatGPT) ----
            # The node clamp zeros φ. But ∇φ ≠ 0.
            # The star doesn't couple to amplitude.
            # It couples to PHASE STRUCTURE — the tension
            # in the string, not the sound.
            #
            # Silencing the string is not enough.
            # We must flatten the tension at L1.
            #
            # ∇φ → 0 at L1 means no gradient for the
            # star's m=1 to grab onto. The phase field
            # is locally flat. No shear. No twist.
            # The star sees a mirror-smooth surface.
            gr = ncr + 2
            ix_g = int(np.clip(l1[0], gr+1, nx-gr-2))
            iy_g = int(np.clip(l1[1], gr+1, ny-gr-2))
            phi_grad_patch = phi[ix_g-gr:ix_g+gr,
                                 iy_g-gr:iy_g+gr].copy()
            if phi_grad_patch.shape[0] > 2 and phi_grad_patch.shape[1] > 2:
                # Compute local mean (the "flat" target)
                local_mean = float(np.mean(phi_grad_patch))
                # Flatten: drive every cell toward the mean
                # This kills ∇φ without changing the average
                GRADIENT_KILL = 0.85
                phi_flattened = (phi_grad_patch * (1.0 - GRADIENT_KILL) +
                                 local_mean * GRADIENT_KILL)
                # Energy removed by flattening → chi
                gradient_energy = np.abs(phi_grad_patch - phi_flattened)
                phi[ix_g-gr:ix_g+gr,
                    iy_g-gr:iy_g+gr] = phi_flattened
                chi_field[ix_g-gr:ix_g+gr,
                          iy_g-gr:iy_g+gr] += gradient_energy * G * 0.5
        ...
        # ---- SETTLING PHI DAMPER (v19) ----
        # During settling after the flash, phi is elevated
        # from the shockwave. This is what fogs the mirror
        # (OpT depends on phi_rms). The craft damps its
        # own phi toward baseline. Not gated behind the
        # stellar detector — this is craft management.
        if in_settling:
            phi_rms_settle = float(np.sqrt(np.mean(phi**2)))
            if phi_rms_settle > INTERNAL_PHI_BASELINE * 5:
                settle_damp = min(0.6,
                    (phi_rms_settle / INTERNAL_PHI_BASELINE) * 0.01)
                phi *= (1.0 - settle_damp)
                # Damped energy goes to chi
                chi_field += np.abs(phi) * settle_damp * 0.1
    ...
    out_path=os.path.join(out_dir,
        f"BCM_v20_blackhole_v2_"
        f"{time.strftime('%Y%m%d_%H%M%S')}.json")

    out_data={
        "title":"BCM v20.21 Black Hole Transit (Gradient Kill)",
        "author":"Stephen Justin Burdick Sr. -- "
                 "Emerald Entities LLC",
        "purpose":"Fly through XTE J1650-500.with "
                  "tachocline gate detector active. "
                  "7D fold matrix governs approach.",
        "target":"XTE J1650-500 (3.8 Msun, 24 km)",
        "c_substrate":"12,000c",
        "gate_constants": {
            "dphi_gate": DPHI_GATE,
